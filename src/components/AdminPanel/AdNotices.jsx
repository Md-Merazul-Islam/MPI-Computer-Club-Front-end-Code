...
const API_BASE_URL = 'https://mpi-computer-club-backend.vercel.app/academics';
...
const AdNotices = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append the notice details to the FormData
    formData.append("title", noticeForm.title);
    formData.append("description", noticeForm.description);
  
    // If there is a file to upload, send it to ImgBB first
    let fileUrl = null;
    if (noticeForm.file) {
      fileUrl = await uploadImageToImgBB(noticeForm.file);
      if (fileUrl) {
        formData.append("file", fileUrl); // Append the file URL from ImgBB
      }
    }
    ...
    try {
      ...
      // Fetch notices after creation or update
      fetchNotices();
      ...
      // Optionally set an error message here if you have state for that
    }
  };
